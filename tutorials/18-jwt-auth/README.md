# 第十八課：JWT 認證（JWT Authentication）
...
4. **環境變數**：把 `jwtSecret` 改成從 `os.Getenv("JWT_SECRET")` 讀取，用 `JWT_SECRET=mysecret go run ./tutorials/18-jwt-auth` 執行
