...
| 階段 | 說明 |
|------|------|
| 🟡 中級工程師 | **重點**：寫測試是專業工程師的基本功 |
| 🔴 資深工程師 | TDD 實踐、測試策略（單元/整合/E2E）、測試覆蓋率 |
...
```bash
# 跑所有測試（詳細輸出）
go test -v ./tutorials/19-testing/

# 顯示測試覆蓋率
go test -cover ./tutorials/19-testing/

# 只跑特定測試（名稱包含 TestAdd）
go test -run TestAdd ./tutorials/19-testing/

# 只跑特定子測試（斜線後是子測試名稱）
go test -run TestAdd/正數相加 ./tutorials/19-testing/
```
...
4. **覆蓋率**：執行 `go test -cover ./tutorials/19-testing/`，查看當前覆蓋率，嘗試新增測試讓覆蓋率提高
