# 第二十五課：Goroutine 與並發（Goroutines & Concurrency）
...
| 階段 | 說明 |
|------|------|
| 🟡 中級工程師 | **重點**：Goroutine 是 Go 最大的特色 |
| 🔴 資深工程師 | **必備**：WaitGroup、Channel 模式、避免 Race Condition |
...
```bash
go run ./tutorials/25-goroutines
```

加上競態條件偵測（強烈建議開發時使用）：
```bash
go run -race ./tutorials/25-goroutines
```
